...
const DT = 1 / 15;
const quiet = { nearestDistU: 40, noise: 0, exposed: false, dtS: DT };
...
describe("director", () => {
  ...
  it("sustained loud running triggers a hunt from calm (suspicion ≥ 9)", () => {
    ...
    run(d, { nearestDistU: 15, noise: 0.4, dtS: DT, exposed: false }, SUSPICION_HUNT_AT * 2);
  ...
  it("hunt hard-caps and retreats, and retreat clears suspicion", () => {
    ...
    run(d, { nearestDistU: 15, noise: 0.4, dtS: DT, exposed: false }, SUSPICION_HUNT_AT * 2);
    expect(d.mood).toBe("hunt");
    run(d, { nearestDistU: 12, noise: 0.4, dtS: DT, exposed: false }, HUNT_MAX_S + 1);
  ...
  it("camping pressure: monster near for long → menace valve sends it away (stalk → retreat)", () => {
    ...
    run(d, { nearestDistU: 9, noise: 0, dtS: DT, exposed: false }, 40, rng);
    expect(d.mood).toBe("retreat");
  });

  it("whispering is always safe, a scream is an instant hunt", () => {
    const whisper = makeDirector();
    run(whisper, { nearestDistU: 6, noise: 0.2, dtS: DT, exposed: false }, 60); // under the gate
    expect(whisper.suspicion).toBe(0);
    expect(whisper.mood).not.toBe("hunt");

    const scream = makeDirector();
    run(scream, { nearestDistU: 6, noise: 0.9, dtS: DT, exposed: false }, 1); // one second of screaming
  ...
  it("talking is riskier than footsteps: louder noise hunts sooner", () => {
    ...
    run(talk, { nearestDistU: 12, noise: 0.7, dtS: DT, exposed: false }, 5); // voice: +3/s
    run(steps, { nearestDistU: 12, noise: 0.3, dtS: DT, exposed: false }, 5); // walking: +1/s
    expect(talk.suspicion).toBeGreaterThan(steps.suspicion);
  });

  it("downing a player always forces retreat", () => {
    const d = makeDirector();
    run(d, { nearestDistU: 5, noise: 0.4, dtS: DT, exposed: false }, SUSPICION_HUNT_AT * 2);
    expect(d.mood).toBe("hunt");
  ...
  it("same seed → identical mood trace (determinism)", () => {
    const trace = (seed: number) => {
      ...
      for (let i = 0; i < 200 * 15; i++) {
        tickDirector(d, { nearestDistU: 20 - (i % 300) / 20, noise: i % 90 < 30 ? 0.4 : 0, dtS: DT, exposed: false }, rng);
